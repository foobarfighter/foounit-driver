...
Profile.prototype.run = function (callback){
  this._queue.enqueue(new fu.Block(callback));
}


Profile.prototype.switchTo = function (pageName){
  var path = foounit.translatePath(driver.config().pages)
    + '/' + inflection.underscore(pageName);

  var urls = fixture.load('urls');

  var Klass = require(path)
    , options = urls.pages[pageName];
  options.baseUri = urls.baseUri;

  return new Klass(this, options).doSwitch();
}
